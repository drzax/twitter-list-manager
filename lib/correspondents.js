...
function correspondents(duration) {
	return new Promise((resolve, reject) => {
		...
		timeline = pager(client, 'statuses/user_timeline', {count: 200}, stopAt(earlier))
			.then(filterByTime(earlier))
			.then(filterTimeline)
			.then(compile);
			
		favourites = pager(client,'favorites/list',{count: 200}, stopAt(earlier))
			.then(filterByTime(earlier))
			.then(compile);
		
		Promise.all([timeline,favourites]).then(([timeline, favourites]) => {
...
function filterTimeline(statuses) {
	return statuses.filter((r)=>{
		return !!r.retweeted_status || !!r.quoted_status || r.in_reply_to_screen_name;
	});
}

function filterByTime(time) {
	return (statuses)=>{
		return statuses.filter((status) => moment(status.created_at, 'ddd MMM DD HH:mm:ss ZZ YYYY') >= time);
	};
}

function stopAt(time) {
	return (res) => {
		return moment(res[res.length-1].created_at, 'ddd MMM DD HH:mm:ss ZZ YYYY') < time;
	};
}